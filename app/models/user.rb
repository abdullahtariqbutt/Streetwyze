class User < ApplicationRecord
  ...
  validates :password, format: { with: PASSWORD_REGEX, message: "should be minimum 8 characters long with atleast one Capital and one Special character" }, if: -> {self.password.present?}
